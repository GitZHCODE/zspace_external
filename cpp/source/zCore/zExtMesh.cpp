...
namespace zSpace
{
	...
	ZSPACE_EXTERNAL_INLINE int zExtMesh::checkMemAlloc(bool allocateMemory)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE void zExtMesh::smoothMesh(int numDivisions, bool smoothCorner, zIntArray fixedIndex)
	{
		...
		for (int j = 0; j < numDivisions; j++)
		{
			...
			for (int i = 0; i < numOriginalVertices; i++)
			{
				...
				if (v.onBoundary())
				{
					...
					if (find(begin(fixedIndex), end(fixedIndex), v.getId()) != end(fixedIndex)) continue;
	...
	ZSPACE_EXTERNAL_INLINE int zExtMeshArray::checkMemAlloc(bool allocateMemory)
	{
		try
		{
			if (pointer != nullptr) return 1;
	...
	ZSPACE_EXTERNAL_INLINE int zExtMeshPointerArray::checkMemAlloc(bool allocateMemory)
	{
		try
		{
			if (pointer != nullptr) return 1;
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_createFromArrays(zExtPointArray& _vertexPositions, zExtIntArray& _polyCounts, zExtIntArray& _polyConnects, zExtMesh& out_mesh)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_setMeshVertexColors(zExtMesh& extMesh, zExtColorArray& vertexColors)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_from(char* filePath, zExtMesh& outMesh)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_to(zExtMesh& extMesh, char* filePath)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_fromJSON(zExtJSON& extJson, zExtMesh& outMesh)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_toJSON(zExtMesh& extMesh, zExtJSON& outJson)
	{
		try
		{
			int memoryChk = extMesh.checkMemAlloc(false);
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_fromUSD(zExtUSD& extUsd, zExtMesh& outMesh)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_toUSD(zExtMesh& extMesh, zExtUSD& extUsd)
	{
		try
		{
			printf("\n tousd 0");
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_getMeshsFromMeshArray(zExtMeshArray& inArray, zExtMesh* outMeshes)
	{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_smoothMesh(zExtMesh& objMesh, int level, bool smoothCorner, zExtIntArray& fixedVerts)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_smoothMesh1D(zExtMesh& objMesh, int level, bool smoothCorner, bool flip, zExtIntArray& fixedVerts)
	{
		try
		{
			if (!objMesh.pointer)
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_getEdgeLoops(zExtMesh& objMesh, zExtGraphArray& graphArrayU, zExtGraphArray& graphArrayV)
	{
		try
		{
			if (!objMesh.pointer)
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_getEdgeLoopsGraph(zExtMesh& objMesh, zExtGraph& graphU, zExtGraph& graphV)
	{
		try
		{
			if (!objMesh.pointer)
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_checkPlanarity(zExtMesh& objMesh, float tolerance, int planarityType, bool colorFaces, zExtDoubleArray& outDeviations)
	{
		try
		{
			zFnMesh fn(*objMesh.pointer);
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_getFaceColor(zExtMesh& objMesh, zExtColorArray& extPointArray)
	{
		try
		{
	...
	ZSPACE_EXTERNAL_INLINE int ext_mesh_getGaussianCurvature(zExtMesh& objMesh, zExtDoubleArray& outGaussianCurvature )
	{
		try
		{
