...
namespace zSpace
{
	...
	{
		graphsCount = graphPArray->size();
	}


	ZSPACE_EXTERNAL_INLINE void ext_graphUtil_getGraphData(zExtGraph extGraph, float* vPositions, float* vColors, int* ePairs, float* eColors)
	{
		zObjGraph* graph;
	...
	ZSPACE_EXTERNAL_INLINE void ext_graphUtil_getGraphSequence(zExtGraph extGraph, int* outSequence)//the size of the area = the number of vertices + 1
	{
		zIntArray seq;
	...
	ZSPACE_EXTERNAL_INLINE void ext_graphUtil_getGraphsFromGraphPointerArray(zExtGraphPointerArray graphSet, zExtGraph* outGraphArray)
	{

		for (int i = 0; i < graphSet.graphsCount; i++)
		{
			outGraphArray[i].graph = graphSet.graphPArray->at(i);
	...
	////Graph Data
	//ZSPACE_EXTERNAL_INLINE void ext_graphUtil_getGraphsSetFromPointersVector2(zObjGraphPointerArray* graphs, zObjGraph** outGraphArray)
	//{
	//	for (int i = 0; i < graphs->size(); i++)
	//	{
	...
	//	}
	//}
	//ZSPACE_EXTERNAL_INLINE void ext_graphUtil_getGraphsSetFromVector2(zObjGraphArray* graphs, zObjGraph** outGraphArray)
	//{
	//	for (int i = 0; i < graphs->size(); i++)
	//	{
	//		outGraphArray[i] = &graphs->at(i);
	//	}
	//}
	//ZSPACE_EXTERNAL_INLINE void ext_graphUtil_getGraphCounts2(zObjGraph* graph, int& outvCount, int& outeCount)
	//{
	//	zFnGraph g(*graph);
	//	outeCount = g.numEdges();
	//	outvCount = g.numVertices();
	//}
	//ZSPACE_EXTERNAL_INLINE void ext_graphUtil_getGraphData2(zObjGraph* graph, float* vPositions, float* vColors, int* ePairs, float* eColors)
	//{
	//	zFnGraph g(*graph);
	//	int vCount = g.numVertices();
	//	int eCount = g.numEdges();
