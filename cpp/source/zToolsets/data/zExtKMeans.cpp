...
#include<headers/zToolsets/data/zExtKMeans.h>
...
namespace zSpace
{
	...
	ZSPACE_EXTERNAL_INLINE void ext_KMean_computeKmeans(zTsKMeans*& kmean, double* _data, int dataCount, int strideCount, int& numCluster, int& numIterations, int initMethod, int seed1, int seed2, int* outClusterID)
	{
		//create matrix from 1d data
		MatrixXf data(dataCount, strideCount);
		for (int i = 0; i < dataCount; i++)
	...
	ZSPACE_EXTERNAL_INLINE void ext_KMean_getMeans(zTsKMeans*& kmean, double* outMeans)
	{
		if (!kmean)
		{
			return;
	...
	ZSPACE_EXTERNAL_INLINE int ext_KMean_findOptimalK(double* _data, int dataCount, int strideCount, int& numIterations,  int initMethod,  int seed1, int seed2, int min, int max, int increment, int optimalCountMethod, int* outK, double* outScore)
	{
