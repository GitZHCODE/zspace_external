...
namespace zSpace {
    class Program {
        static void Main(string[] args) {
            ...
            rhMesh.Vertices.Add(new Point3d(0,0,0));
            rhMesh.Vertices.Add(new Point3d(1,0,0));
            rhMesh.Vertices.Add(new Point3d(1,1,0));

            rhMesh.Faces.AddFace(0, 1, 2);
            rhMesh.Compact();
            rhMesh.Normals.ComputeNormals();

            extMesh.from(rhMesh);

            zExtGraphArray contours = new zExtGraphArray();
            int[] startIds = new int[1] { 0 };
            int[] endIds = new int[1] { 2 };
            zExtIntArray _startIds = new zExtIntArray();
            zExtIntArray _endIds = new zExtIntArray();
            _startIds.setItems(startIds);
            _endIds.setItems(endIds);
            extMesh.computeGeodesicContour(out contours, _startIds, _endIds, 20);

            Console.WriteLine($"contours.getCount: {contours.getCount()}");
